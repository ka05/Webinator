  ...
  // dismiss popover
    ...
    if(lsUserNameAndId){
      ...
      var accountBar = '<div class="btn-group">'
                      ...
                      + '<li><a href="#" onclick="handleMenu(\'account details\')">Account</a></li>'
...
function addPageToProject(){
  ...
  createNewPage(function(){
      // callback function
      addPageCallBack(PAGE.PAGEID, PAGE.PAGENAME);
      $('#pagelink-' + PAGE.PAGEID).addClass('active'); // make current tab active
      $('#template-page-cont').html(''); // clear out page view
  });
}

function addPageCallBack(_pageId, _pageName){
    $('<li role="presentation" class="page-links" data-link-id="' + _pageId + '" id="pagelink-' + _pageId + '" onclick="getCurrPage(event, $(this));"><a href="#" role="tab" data-toggle="tab">' + _pageName + '<span data-id="page-' + _pageId + '-wrap" class="close-x-btn" onclick="promptRemovePage($(this));">×</span></a></li>').insertBefore($('#add-tab-btn'));
}

function getCurrPage(e, _ele){
  if (e.target.nodeName == "span") {
    // e.stopPropagation();
  }
  else {
    removeActiveClasses(_ele);
    getPage(_ele.attr('data-link-id'));
  }
}
...
function removeActiveClasses(_ele){
    ...
    if(_ele){
        ...
        //set activePage
        PAGE.PAGEID = _ele.attr('data-link-id');
    }
}
...
function showEventModal(_ele){
    ...
    $("#modal-body").html('<div>\
                                ...
                                <span class="input-group-addon" style="padding:0px;">\
                                    <input type="button" value="Add ID" class="btn btn-primary" onclick="addID($(this))">\
                                </span></div>\
                                <hr>\
                                <div>\
...
function preCutEle(){
  currEle.resizable("destroy");
  currEle.popover("hide");
}

function cutElement(_currEle){
  preCutEle();
  cutEle = _currEle.clone(cutEle);
  PROCESSDOCUMENT.append();
  console.log(PROCESSDOCUMENT.html());
...
function pasteElement(){
  if(cutEle){
    $('#template-page-cont').append(cutEle);
    initDraggable(cutEle);
